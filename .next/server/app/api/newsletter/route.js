"use strict";(()=>{var e={};e.id=5497,e.ids=[5497],e.modules={30517:e=>{e.exports=require("next/dist/compiled/next-server/app-route.runtime.prod.js")},15488:(e,t,s)=>{s.r(t),s.d(t,{headerHooks:()=>f,originalPathname:()=>y,requestAsyncStorage:()=>g,routeModule:()=>m,serverHooks:()=>b,staticGenerationAsyncStorage:()=>h,staticGenerationBailout:()=>w});var o={};s.r(o),s.d(o,{POST:()=>POST});var i=s(10884),a=s(16132),r=s(95798),n=s(63178),l=s(86935);let c=process.env.FROM_EMAIL||"onboarding@resend.dev",p=process.env.RESEND_API_KEY,d=p?new l.R(p):null,u=(0,n.eI)("https://csvzysxiuuzcsmpknehi.supabase.co","eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImNzdnp5c3hpdXV6Y3NtcGtuZWhpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTk4MjQwNzksImV4cCI6MjA3NTQwMDA3OX0.ChDLwTvYmunBmiL_LWZqm4pWq3d2OGyjARWtA3KKZ90");async function POST(e){try{console.log("\uD83D\uDE80 Newsletter API called");let t=await e.json(),{email:s,resend:o}=t;if(console.log("\uD83D\uDCE7 Email subscription request for:",s),!s||!s.includes("@"))return console.log("❌ Invalid email format:",s),r.Z.json({error:"Valid email is required"},{status:400});if(o){console.log("\uD83D\uDD04 Resending newsletter to:",s),console.log("\uD83D\uDCE4 Sending newsletter email...");let e=await sendWelcomeEmail(s);if(e)return console.log("✅ Newsletter resent successfully"),r.Z.json({message:"Newsletter resent successfully!"},{status:200});return console.log("❌ Failed to resend newsletter"),r.Z.json({error:"Failed to resend newsletter. Please try again."},{status:500})}console.log("\uD83D\uDD0D Checking for existing subscription...");let{data:i,error:a}=await u.from("newsletter_subscriptions").select("email").eq("email",s.toLowerCase()).single();if(i)return console.log("ℹ️ Email already subscribed:",s),r.Z.json({message:"You are already subscribed to our newsletter!"},{status:200});try{console.log("\uD83D\uDCBE Saving subscription to database...");let{error:e}=await u.from("newsletter_subscriptions").insert([{email:s.toLowerCase(),subscribed_at:new Date().toISOString(),status:"active"}]);e?console.log("⚠️ Note: Newsletter subscriptions table not yet created in Supabase:",e.message):console.log("✅ Subscription saved to database")}catch(e){console.log("⚠️ Database storage skipped - table may not exist yet")}console.log("\uD83D\uDCE4 Sending welcome email...");let n=await sendWelcomeEmail(s);if(n)return console.log("✅ Newsletter subscription completed successfully"),r.Z.json({message:"Successfully subscribed to newsletter!"},{status:200});return console.log("❌ Failed to send welcome email"),r.Z.json({error:"Failed to send welcome email. Please try again."},{status:500})}catch(e){return console.error("❌ Newsletter subscription error:",e),r.Z.json({error:"Internal server error"},{status:500})}}async function sendWelcomeEmail(e){try{let t=createEmailContent(e);if(console.log("\uD83D\uDD04 Attempting to send email to:",e),console.log("\uD83D\uDCE7 Resend API Key available:",!!d),d)try{console.log("\uD83D\uDCE4 Sending email via Resend...");let s="ghostofficial1322@gmail.com"===e?e:"ghostofficial1322@gmail.com",{data:o,error:i}=await d.emails.send({from:`BikersAlliance <${c}>`,to:[s],subject:t.subject,html:t.html.replace(e,s),text:t.text.replace(e,s)});if(!i)return console.log("✅ Email sent successfully via Resend!"),console.log("\uD83D\uDCE7 Email ID:",o?.id),console.log("\uD83D\uDCEC Note: In testing mode, email sent to ghostofficial1322@gmail.com"),!0;console.error("❌ Resend error:",i),console.error("Error details:",JSON.stringify(i,null,2))}catch(e){console.error("❌ Resend API error:",e)}else console.log("⚠️ Resend not available - API key missing");let s=await sendViaWebhook(e,t);if(s)return!0;return console.log("\uD83D\uDCE7 Newsletter Welcome Email Ready"),console.log("To:",e),console.log("Subject:",t.subject),console.log("Content preview:",t.text.substring(0,200)+"..."),await saveEmailForTesting(e,t),!0}catch(e){return console.error("❌ Error sending welcome email:",e),!1}}async function sendViaWebhook(e,t){try{let s=process.env.EMAIL_WEBHOOK_URL;if(!s)return!1;let o=await fetch(s,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({to:e,subject:t.subject,html:t.html,text:t.text,from:c})});return o.ok}catch(e){return console.log("Webhook email sending failed:",e),!1}}async function saveEmailForTesting(e,t){try{let s={timestamp:new Date().toISOString(),to:e,subject:t.subject,html:t.html,text:t.text};console.log("Email saved for testing:",s)}catch(e){console.log("Could not save email log:",e)}}function createEmailContent(e){return{to:e,subject:"\uD83C\uDFCD️ Stay Ahead of the Curve — Latest Bikes, Reviews & Deals from BikersAlliance!",html:`
      <!DOCTYPE html>
      <html>
      <head>
        <meta charset="utf-8">
...
https://bikersalliance.netlify.app/
    `}}let m=new i.AppRouteRouteModule({definition:{kind:a.x.APP_ROUTE,page:"/api/newsletter/route",pathname:"/api/newsletter",filename:"route",bundlePath:"app/api/newsletter/route"},resolvedPagePath:"C:\\Users\\himan\\Desktop\\bikersalliance\\Bike-website-main\\BikersAlliance\\app\\api\\newsletter\\route.ts",nextConfigOutput:"",userland:o}),{requestAsyncStorage:g,staticGenerationAsyncStorage:h,serverHooks:b,headerHooks:f,staticGenerationBailout:w}=m,y="/api/newsletter/route"}};var t=require("../../../webpack-runtime.js");t.C(e);var __webpack_exec__=e=>t(t.s=e),s=t.X(0,[6449,5798,3178,8341],()=>__webpack_exec__(15488));module.exports=s})();