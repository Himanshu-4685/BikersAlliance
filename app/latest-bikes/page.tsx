...
import { FiChevronRight, FiSettings, FiBarChart, FiTag, FiFilter, FiX } from 'react-icons/fi';
import { BikeStatus } from '@/types/bike-status';

// Types
interface LatestBike {
  id: string;
  name: string;
  image: string;
  price: string;
...
export default function LatestBikesPage() {
  ...
  useEffect(() => {
    const fetchBikes = async () => {
      try {
        setLoading(true);
        const response = await fetch('/api/bike-status?status=new_launch&limit=50');
        ...
        console.log('Latest Bikes API Response:', data);
        
        if (data.success && data.data) {
          console.log('Setting latest bikes data:', data.data);
          setBikes(data.data);
        } else {
          console.log('No latest bikes data found, setting error');
          setError('No latest bikes found');
        }
      } catch (err) {
        console.error('Error fetching latest bikes:', err);
      ...
      <div className="bg-white border-b">
        <div className="container py-6">
          <div className="flex items-center text-sm text-gray-500 mb-4">
            ...
            <span className="text-gray-900">Latest Bikes</span>
          </div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">
            Latest Bikes & Scooters
          </h1>
          <p className="text-gray-600">
            {filteredBikes.length} latest {filteredBikes.length === 1 ? 'bike' : 'bikes'} launched recently
          </p>
        </div>
      </div>
          ...
          <div className="lg:col-span-3">
            {loading ? (
                  ...
                  >
                    ...
                    <Link href={`/bikes/${bike.variant.slug}`} className="block">
                      <div className="relative h-48 bg-gray-100">
                        ...
                        />
                        <div className="absolute top-0 left-0 px-2 py-1 text-xs font-medium text-white bg-green-600">
                          New Launch
                        </div>
                    ...
                    <div className="p-4">
                      ...
                      <div className="mb-3">
                        <div className="text-lg font-bold text-gray-900 mb-1">
                          ₹ {bike.variant.onRoadPrice ? bike.variant.onRoadPrice.toLocaleString() : bike.priceRange}
                        </div>
                        <div className="text-sm text-gray-500">Ex-showroom Price</div>
                      </div>

                      {/* Launch Date */}
                      <div className="flex items-center pt-3 mt-3 text-sm text-gray-500 border-t border-gray-100 mb-3">
                        <span>Launched: {bike.launchDate || 'Recently'}</span>
                      </div>
                      ...
                      <div className="grid grid-cols-3 gap-2 mb-4 text-xs">
                        <div className="text-center p-2 bg-gray-50 rounded">
                          <FiSettings className="w-4 h-4 mx-auto mb-1 text-gray-500" />
                          <div className="font-medium text-gray-900">{bike.variant.specs?.displacement || 'N/A'}</div>
                          <div className="text-gray-500">Engine</div>
                        </div>
                        <div className="text-center p-2 bg-gray-50 rounded">
                          <FiBarChart className="w-4 h-4 mx-auto mb-1 text-gray-500" />
                          <div className="font-medium text-gray-900">{bike.variant.specs?.peak_power || 'N/A'}</div>
                          <div className="text-gray-500">Power</div>
                        </div>
                        <div className="text-center p-2 bg-gray-50 rounded">
                          <div className="w-4 h-4 mx-auto mb-1 text-gray-500 font-bold">⛽</div>
                          <div className="font-medium text-gray-900">{bike.variant.specs?.city_mileage || 'N/A'}</div>
              ...
              <div className="flex flex-col items-center justify-center p-12 text-center bg-white rounded-lg">
                ...
                <h3 className="text-lg font-medium text-gray-900 mb-2">No latest bikes found</h3>
                <p className="text-gray-600 mb-4">
                  Try adjusting your filters to see more latest bikes.
                </p>
                <button 
                  onClick={clearFilters}
                  className="px-4 py-2 text-sm text-primary border border-primary rounded-md hover:bg-primary hover:text-white transition-colors"
