...
export default function ShortlistedVehiclesPage() {
  ...
  const cleanSlug = (slug: string) => {
    ...
    // First try double dash separation
    if (slug.includes('--')) {
      const parts = slug.split('--');
      return parts[parts.length - 1] || slug;
    }
    
    // Handle duplicate brand/model names in slug (e.g., ducati-panigale-ducati-panigale-v4-r)
    const parts = slug.split('-');
    const uniqueParts = [];
    const seen = new Set();
    
    for (const part of parts) {
      if (!seen.has(part.toLowerCase()) || part.toLowerCase().match(/^(v\d|r|s|pro|plus|\d+)$/)) {
