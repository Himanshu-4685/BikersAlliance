...
export async function GET(
  ...
  try {
    ...
    // Build the query to get bikes by body type
    let query = supabase
      .from('variants')
      .select(`
        variant_id,
    ...
    const formattedBikes = (variants || []).map((variant: any) => ({
      ...
      brand: {
        name: variant.brands?.brand_name || '',
        logo: variant.brands?.logo_url || null
      },
      model: variant.models?.model_name || '',
      image: variant.images?.[0]?.url || null,
      price: variant.on_road_price,
      specs: {
        bodyType: variant.specs?.body_type || '',
        engine: variant.specs?.engine_type || '',
        displacement: variant.specs?.displacement || '',
        power: variant.specs?.peak_power || '',
        mileage: variant.specs?.city_mileage || variant.specs?.highway_mileage || ''
      }
    }));
