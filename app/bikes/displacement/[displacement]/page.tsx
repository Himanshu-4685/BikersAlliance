...
export default function DisplacementFilterPage() {
  const params = useParams();
  const searchParams = useSearchParams();
  const router = useRouter();
  const displacementSlug = params.displacement as string;

  // State
  const [bikes, setBikes] = useState<Bike[]>([]);
  const [displacementInfo, setDisplacementInfo] = useState<DisplacementInfo | null>(null);
  ...
  const fetchBikes = async () => {
    try {
      ...
      const response = await fetch(`/api/bikes/displacement/${displacementSlug}?${params}`);
      ...
      setBikes(data.bikes);
      setDisplacementInfo(data.displacement);
  ...
  useEffect(() => {
    fetchBikes();
  }, [displacementSlug, selectedBrand, selectedBodyType, minPrice, maxPrice, searchQuery, sortBy, sortOrder, pagination.page]);
        ...
        <div className="container mx-auto px-4 py-3">
          <nav className="flex items-center space-x-2 text-sm">
            ...
            <span className="text-primary font-medium">
              {displacementInfo?.label || 'Displacement Filter'}
            </span>
          </nav>
        </div>
      </div>
          ...
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">
                {displacementInfo?.label} Bikes
              </h1>
              <p className="text-gray-600 mt-2">
                {displacementInfo?.description}
              </p>
        ...
        <div className="flex flex-col md:flex-row gap-6">
          ...
          <div className="md:w-3/4">
            {loading ? (
              ...
              <div className="text-center py-12">
                <p className="text-gray-600">No bikes found with this displacement.</p>
                <button
                  onClick={clearFilters}
                  className="mt-4 px-4 py-2 bg-primary text-white rounded-md hover:bg-primary-600"
                >
