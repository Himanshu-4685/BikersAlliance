...
export default function BikeDetailsPage() {
  ...
  const prevImage = () => {
    if (bike && bike.images.length > 0) {
      setActiveImage((prev) => (prev - 1 + bike.images.length) % bike.images.length);
    }
  };

  // Group specifications by category
  const groupedSpecs = bike ? bike.specifications.reduce((acc, spec) => {
    if (!acc[spec.category]) {
      acc[spec.category] = [];
    }
    acc[spec.category].push(spec);
    return acc;
  }, {} as Record<string, typeof bike.specifications>) : {};
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-6">
              ...
              <div className="flex items-center space-x-4 text-sm text-gray-600">
                {bike.rating && bike.rating.count > 0 && (
                  <div className="flex items-center">
                    <div className="flex items-center mr-1">
                      {[...Array(5)].map((_, i) => (
                        <FiStar
                          key={i}
                          className={`w-4 h-4 ${
                            i < Math.floor(bike.rating!.average)
                    ...
                    <span className="font-medium">{bike.rating.average}</span>
                ...
                {bike.launchDate && (
                  <div className="flex items-center">
                    ...
                    <span>Launched: {new Date(bike.launchDate).toLocaleDateString('en-US', { 
                      month: 'long', 
                      year: 'numeric' 
                    })}</span>
                  </div>
          ...
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
              <div className="relative aspect-video">
                <Image
                  src={bike.images[activeImage]?.url || '/images/placeholder-bike.jpg'}
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center p-4 bg-primary/5 rounded-lg">
                    ...
                    <div className="text-3xl font-bold text-primary">
                      {formatPrice(bike.variants[selectedVariant]?.price || bike.variants[0]?.price || 0)}
                    </div>
              ...
              <div className="space-y-3">
                {bike.brand && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900">{bike.brand.name}</span>
                  </div>
                )}
        ...
        {similarModels.length > 0 && (
          <div className="mt-12">
            <h2 className="text-2xl font-bold text-gray-900 mb-6">Similar Models</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {similarModels.slice(0, 6).map(model => (
                <Link key={model.id} href={`/bikes/${model.slug}`}>
